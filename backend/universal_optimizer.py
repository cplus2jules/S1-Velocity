...
class UniversalFishSpeechOptimizer:
    ...
    def synthesize(self, text: str, reference_audio: str = None, **kwargs):
        """Universal synthesis method with Algorithm 3.2 Dynamic Chunking"""
        start_time = time.time()
        
        # Algorithm 3.2: Dynamic Chunking Selection
        opt = self.config['optimizations']
        chunk_length = kwargs.get('chunk_length', opt.get('chunk_length', 0))
        
        # Automatic determination based on memory if not specified
        if chunk_length == 0:
            mem_gb = self.config['hardware_info']['memory_gb']
            if mem_gb < 8:
                chunk_length = 50  # Conservative for 4GB/8GB
            elif mem_gb < 16:
                chunk_length = 150 # Balanced
            else:
                chunk_length = 300 # Aggressive for 16GB+
        
        # Decide synthesis path
        if len(text) > chunk_length and chunk_length > 0:
            logger.info(f"[STRATEGY] Large text detected ({len(text)} chars), using Algorithm 3.2 chunking (L={chunk_length})")
            result = self._chunked_synthesis(text, reference_audio, **kwargs)
        else:
            logger.info(f"[STRATEGY] Using direct synthesis (Len: {len(text)} <= Chunk: {chunk_length})")
