...
class SynthesisMetrics:
    ...
    msc: float = 0.0  # Memory Stability Coefficient (1 - std/mean)
    ...
    def finalize(self, memory_snapshots: List[MetricSnapshot], gpu_snapshots: List[MetricSnapshot]):
        ...
        # Calculate MSC (Memory Stability Coefficient)
        if len(memory_snapshots) > 1:
            mem_values = [s.memory_mb for s in memory_snapshots]
            mean_mem = sum(mem_values) / len(mem_values)
            if mean_mem > 0:
                # Calculate standard deviation
                variance = sum((x - mean_mem) ** 2 for x in mem_values) / len(mem_values)
                std_dev = variance ** 0.5
                self.msc = 1.0 - (std_dev / mean_mem)
                self.msc = max(0.0, min(1.0, self.msc))
            else:
                self.msc = 1.0
        elif len(memory_snapshots) == 1:
            self.msc = 1.0
