    ...
    def synthesize(self, text: str, reference_audio: Optional[str] = None, **kwargs) -> Tuple[np.ndarray, int, Dict[str, Any]]:
        ...
        try:
            ...
            if self.tokenizer is None:
                import sys
                # Add fish-speech to path if needed for internal imports
                ...
                # Note: FishTokenizer expects the path to the model folder containing tokenizer.json
                self.tokenizer = FishTokenizer(str(self.model_path))
            
            encoded = self.tokenizer.encode(text)
            input_ids = np.array([encoded], dtype=np.int64)
            attention_mask = np.ones_like(input_ids)
            
            # Use specific providers for ONNX
            providers = ['CPUExecutionProvider']
