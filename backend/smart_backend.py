...
class ConfigurationSelector:
    ...
    def _m1_air_config(self) -> OptimalConfig:
        ...
        # FORCED CPU MODE: MPS is currently too slow for this model architecture on M1 Air
        device = 'cpu'
        
        # ============================================
        # CRITICAL: Set macOS-specific environment variables for performance
        # ============================================
        os.environ["OMP_NUM_THREADS"] = "8"
        os.environ["MKL_NUM_THREADS"] = "8"
        logger.info("[OK] M1 Air: Environment variables configured for CPU performance")
        logger.info("   OMP_NUM_THREADS=8, MKL_NUM_THREADS=8")
        
        return OptimalConfig(
            device=device,
            precision='bf16', # bfloat16 is more stable on CPU than float16
            quantization='int8', 
            useonnx=False,
            usetorchcompile=False, # Disable for CPU stability
            chunk_length=512, # Smaller chunks for less memory pressure
            max_batch_size=1,
            num_threads=8, # Use all cores for CPU inference
            cache_limit=25,
            enable_thermal_management=True,
            expected_rtf=10.0, 
            expected_memory_gb=1.8,
            optimization_strategy='m1_air_cpu_optimized',
            notes=f'INT8 weight-only + FP16 compute forced on CPU for M1 Air stability.',
...
class SmartAdaptiveBackend:
    ...
    def _initialize_engine(self, model_path: str):
        ...
        try:
            ...
            from .opt_engine_v2 import OptimizedFishSpeechV2
        logger.info(f"[LOAD] Loading Standard Optimized Engine V2 (Quant: {self.config.quantization})")
